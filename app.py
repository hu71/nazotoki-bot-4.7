...
def handle_text(event):
    ...
    print(f"Received text from user_id: {user_id}, text: {text}")  # デバッグログ
...
def handle_image(event):
    ...
    print(f"Received image from user_id: {user_id}")  # デバッグログ
...
def judge():
    ...
    if request.method == "POST":
        ...
        qnum_str = request.form.get("qnum", "")
        result = request.form.get("result", "")

        print(f"Received form data: user_id={user_id}, qnum={qnum_str}, result={result}")  # デバッグログ

        # user_idのバリデーション
        if not user_id or not isinstance(user_id, str) or len(user_id) < 10 or not user_id.startswith("U"):
            print(f"Invalid user_id: {user_id} (must be a string starting with 'U' and at least 10 chars)")
        ...
        try:
            ...
            if qnum < 0 or qnum >= len(questions):
                print(f"Invalid qnum: {qnum_str} (out of range 0-{len(questions)-1})")
                return "Invalid question number", 400
        except ValueError as e:
            print(f"ValueError in judge: {str(e)} - qnum_str was {qnum_str}")
            return "Invalid question number format", 400

        # 必須フィールドのチェック
        if not result:
            print(f"Missing required field: result={result}")
        ...
        try:
            ...
            print(f"Failed to send result to {user_id}: {str(e)} - Status code: {getattr(e, 'status_code', 'N/A')}, Error details: {getattr(e, 'error_details', 'N/A')}")
